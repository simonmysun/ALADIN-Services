...
#   YPS1=Mpar.YPS1;                    #%Ausbeutekoeffizient Produkt aus Substrat 1 je g Produkt
    ...
    Luft_In=Fpar["Q_Air"]*60       #Umrechnung ZuLuftstrom Luft von NL/min in NL/h
    ...
    if mu > 1*10^(-3): #wenn mu größer ~0 dann keine nicht wachstumsassoziierte Produktbildung; bei numerischer Integration wird mu nie exakt 0
        ...
        b = Mpar["beta"]*c_S1/(Mpar["KMS1"]+c_S1)


    rp=Mpar["Prod"]*c_x*(mu*Mpar["alpha"]+b) #Luedking Piret
    ...
    #Begasung / Sauerstoff
    CER=OUR*Mpar["RQ_x"]+(rp/Mpar["Y_CO2_P"])/44 #Divison durch 44: Umrechung von g/L in mol/L
    Luft_Out=Luft_In+(rp/Mpar["Y_CO2_P"])/44*Vm_norm #Volumen der Abluft ist um entstandenes CO2 höher

#   Biomassebildung
    cx_r=mu*c_x

#Substratverbrauchsgeschwindigkeit Substrat 1 durch Biomassebildung und Produktbildung
    c_S1_r=c_x*(-mu/Mpar["YXS1"]-Mpar["Prod"]*(mu*Mpar["alpha"]+b)/Mpar["YPS1"])+Fpar["Feed_C"]
