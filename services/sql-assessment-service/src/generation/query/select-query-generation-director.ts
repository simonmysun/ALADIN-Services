...
import { IParsedColumn, IParsedTable, IPath, ITaskConfiguration } from '../../shared/interfaces/domain';
...
export class SelectQueryGenerationDirector implements QueryGenerationDirector {
    ...
    public async buildQuery(
        ...
        if (context.joinDepth > 0) {
            ...
            mergedColumns = this.selectColumnsFromJoinPath(randomTable, metadata, joinPath, context.joinDepth);
        } else {
        ...
        if (mergedColumns.length < context.predicateCount || mergedColumns.length < context.columnCount) {
            ...
            return await this.buildQuery(context, metadata, filteredTables, datasource, schema, shuffleCounter + 1);
        }
        ...
        if (context.joinDepth > 0 && randomTable.joinPaths.length > 0) {
            this.astBuilder.buildFromWithJoin(schema, randomTable.name, joinPath, context.joinTypes, context.joinDepth);
        } else {
            this.astBuilder.buildFrom(schema, randomTable.name);
        }

        if (context.predicateCount > 0) {
            const missingTypes = this.areAllPredicateTypesAvailable(context, mergedColumns);
            ...
            try {
                const constraints = await this.predicateService.generatePredicates(
                    mergedColumns, context.predicateCount, datasource, schema, context.operationTypes
                );
        ...
        if (context.groupby) {
            try {
                groupByColumn = context.having
                    ? await this.predicateService.generateHavingClauseAndReturnGroupByColumn(mergedColumns, datasource, schema)
                    : this.predicateService.generateGroupByClauseAndReturnColumn(mergedColumns);
            } catch {
                console.log(`Shuffle ${shuffleCounter}`);
                return await this.buildQuery(context, metadata, filteredTables, datasource, schema, shuffleCounter + 1);
            }
            ...
            if (context.columnCount > 1 && context.aggregation) {
                const reducedColumns = this.prepareReducedColumnsForSelectClause(mergedColumns, context.columnCount, context.aggregation);
        ...
        if (context.orderby) {
            if (aggregationSelect) {
                ...
                randomBoolean()
                    ? this.generateOrderByClause([groupByColumn], false, true)
                    : this.generateOrderByClause(mergedColumns, true, false);
            } else {
    ...
    private areAllAggregationColumnsAvailable(mergedColumns: IParsedColumn[], columnCount: number): boolean {
    ...
    private areAllPredicateTypesAvailable(context: ITaskConfiguration, mergedColumns: IParsedColumn[]): string[] {
        ...
        for (const operation of context.operationTypes) {
            ...
            if (operation === 'IS_NULL') {
                column = mergedColumns.find(col => col.isNullable && !usedColumns.has(col.name));
            } else {
                column = mergedColumns.find(col => operationTypes[operation].includes(col.type) && !usedColumns.has(col.name));
            }
    ...
    private selectColumnsFromJoinPath(
        ...
        for (let i = 0; i < depth; i++) {
            const nextTableInPath = allTables.find(table => table.name == path[i].tableName);
            if (nextTableInPath) {
     ...
     */
    private selectJoinPath(randomTable: IParsedTable, joinDepth: number, joinTypes: joinType[]): IPath[] {
        let filteredPaths;
        if (joinTypes.includes('SELF JOIN')) {
            ...
            filteredPaths = randomTable.joinPaths.filter(path => path?.depth >= joinDepth);
        }
        ...
        if (aggregation) {
            ...
            if (aggregatableColumns.length >= columnCount) {
                ...
                reducedColumns.forEach(column => {
                    column.aggregation = this.predicateService.returnAggregateType(column);
                });
            }
        } else {
    ...
    private generateOrderByClause(mergedColumns: IParsedColumn[], aggregate = false, groupby = false) {
        ...
        if (aggregate) {
            ...
            if (aggregatedOrderbyColumn) {
                const aggType = this.predicateService.returnAggregateType(aggregatedOrderbyColumn);
            ...
            const orderbyColumn: IParsedColumn | undefined = shuffle(mergedColumns).find(column =>
                orderableTypes.includes(column.type)
            );
            if (!orderbyColumn) return;
            const currentAST = this.astBuilder.getGeneratedAST();
            let tablename: string = (groupby && currentAST?.groupby?.columns?.length && currentAST.groupby?.columns?.length > 0)
